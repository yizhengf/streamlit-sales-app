...
def read_file(file):
    try:
        ...
        st.warning(f"⚠️ Failed to read {file.name}: {str(e)}")  # 🔹 修改1：添加错误日志
        return None
...
def main():
    ...
    df_list = [read_file(file) for file in uploaded_files]  # 🔹 修改1：read_file 可能返回 None

    # 🔹 修改2：检查 df_list 是否为空
    df_list = [df for df in df_list if df is not None]
    if not df_list:
        st.error("❌ No valid files were uploaded or all files failed to load.")
        return
